...
void OpenglWindow::draw(){
        ...
        int size = oh->edge_list_pers.size();
        for(int i=0;i<size;i++){
            glBegin(GL_LINES);
                glVertex3d((oh->edge_list_pers[i]->p)->x,(oh->edge_list_pers[i]->p)->y,(oh->edge_list_pers[i]->p)->z);
                glVertex3d((oh->edge_list_pers[i]->q)->x,(oh->edge_list_pers[i]->q)->y,(oh->edge_list_pers[i]->q)->z);
            glEnd();
        }
        //qDebug()<<"Size "<<size;

}
void OpenglWindow::draw_front(){

        int size = oh->edge_list_pers.size();
        for(int i=0;i<size;i++){
            glBegin(GL_LINES);
                glVertex3d((oh->edge_list_pers[i]->p)->x,(oh->edge_list_pers[i]->p)->y,(double)0);
                glVertex3d((oh->edge_list_pers[i]->q)->x,(oh->edge_list_pers[i]->q)->y,(double)0);
            glEnd();
        }
       // qDebug()<<"Size "<<size;

}
void OpenglWindow::draw_top(){

        int size = oh->edge_list_pers.size();
        for(int i=0;i<size;i++){
            glBegin(GL_LINES);
                glVertex3d((oh->edge_list_pers[i]->p)->x,(double)0,(oh->edge_list_pers[i]->p)->z);
                glVertex3d((oh->edge_list_pers[i]->q)->x,(double)0,(oh->edge_list_pers[i]->q)->z);
            glEnd();
        }
       // qDebug()<<"Size "<<size;

}
void OpenglWindow::draw_side(){

        int size = oh->edge_list_pers.size();
        for(int i=0;i<size;i++){
            glBegin(GL_LINES);
                glVertex3d((double)0,(oh->edge_list_pers[i]->p)->y,(oh->edge_list_pers[i]->p)->z);
                glVertex3d((double)0,(oh->edge_list_pers[i]->q)->y,(oh->edge_list_pers[i]->q)->z);
            glEnd();
        }
        //qDebug()<<"Size "<<size;
